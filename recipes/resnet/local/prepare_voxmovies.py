...
def prepare_voxmovies(in_data: Pathlike, out_data: Pathlike, jobs: int, sampling_frequency: int, delete_zip: bool):
    ...
    test_lst = get_all_files(in_data / "voxmovies_test", match_and=[".wav"])
    train_lst = get_all_files(in_data / "voxmovies_train", match_and=[".wav"])

    with ProcessPoolExecutor(jobs) as ex:
        futures = []

        for segment in test_lst:
            futures.append(ex.submit(process_file, segment, sampling_frequency, in_data))
        for segment in train_lst:
            futures.append(ex.submit(process_file, segment, sampling_frequency, in_data))
    ...
    for txt, trials in zip( ["e1.txt", "e2.txt", "e3.txt", "e4.txt", "e5.txt"], ["voxmovies-e1.trials", "voxmovies-e2.trials", "voxmovies-e3.trials", "voxmovies-e4.trials", "voxmovies-e5.trials"]):
        r_txt = open(in_data / 'evaluation_sets' / txt, "r")
    ...
    if delete_zip:
        os.remove(in_data / "vox_movies.tar.gz")
