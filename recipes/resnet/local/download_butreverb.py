...
BUT_REVERB_PARTS_URL = [
    ["http://merlin.fit.vutbr.cz/ReverbDB/BUT_ReverbDB_rel_19_06_RIR-Only.tgz", "3ad91777f6cadec81fb4c07dfbef6f19"],
]

def download_butreverb(target_dir: Pathlike = ".", force_download: Optional[bool] = False, check_md5: Optional[bool] = False, num_jobs: int = 10):
    ...
    if zip_path.exists() and not force_download:
        ...
        for url in BUT_REVERB_PARTS_URL:
            ...
            if not fname.exists() and not force_download:
                urlretrieve_progress(url[0], filename=target_dir / url[0].split("/")[-1], desc=f"Downloading BUT REVERB {url[0].split('/')[-1]}")
            elif force_download :
                urlretrieve_progress(url[0], filename=target_dir / url[0].split("/")[-1], desc=f"Downloading BUT REVERB {url[0].split('/')[-1]}")
...
if __name__ == '__main__':
    ...
    download_butreverb(args.target_dir, args.force_download, args.check_md5, args.num_jobs)
