...
def process_file(segment: Pathlike, sampling_frequency: int, out_data: Pathlike):
    ...
    if sampling_frequency != 16000:
        out = Path( out_data / str(segment).split("/")[-5]  /  str(segment).split("/")[-4]  / str(segment).split("/")[-3] / str(segment).split("/")[-2] )
        out.mkdir(parents=True, exist_ok=True)

        output = Path( out_data / str(segment).split("/")[-5]  /  str(segment).split("/")[-4]  / str(segment).split("/")[-3] / str(segment).split("/")[-2] /  str(segment).split("/")[-1]  )
        cmd = "ffmpeg -y -threads 1 -i " + str(segment) + " -acodec pcm_s16le -ac 1 -ar " + str(sampling_frequency) + " -ab 48 -threads 1 " + str(output)
...
def prepare_rirs_noises(in_data: Pathlike, out_data: Pathlike, sampling_frequency: int, num_jobs: int):
...
if __name__ == '__main__':
    ...
    prepare_rirs_noises(args.in_data, args.out_data, args.downsampling, args.num_jobs)
