...
def process_file(segment: Pathlike, out_data: Pathlike):
    name = str(segment).split("/")[-6]+"_"+str(segment).split("/")[-5]+"_"+str(segment).split("/")[-4]+"_"+str(segment).split("/")[-3]+"_"+str(segment).split("/")[-2]+"_"+str(segment).split("/")[-1].split(".")[0]+"_"+str(segment).split("/")[-1].split(".")[1]
    ...
    #cmd = "ffmpeg -y -threads 1 -i " + str(segment) + " -acodec pcm_s16le -ac 1 -ar 16000 -ab 48 -threads 1 " + str(output)
...
def prepare_butreverb(in_data: Pathlike = ".", out_data: Pathlike = ".", num_jobs: int = 20):
    ...
    with ProcessPoolExecutor(num_jobs) as ex:
        ...
        for future in tqdm( futures, total=len(futures), desc="Processing BUT REVERB..."):
...
if __name__ == '__main__':
    ...
    prepare_butreverb(Path(args.in_data), Path(args.out_data), 30)
