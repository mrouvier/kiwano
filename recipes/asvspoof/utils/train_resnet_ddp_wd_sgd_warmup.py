...
if __name__ == '__main__':
    ...
    scheduler = WarmupLRScheduler(optimizer, args.init_lr, args.peak_lr, args.warmup_steps)
    if args.checkpoint:
    ...
    for epochs in range(epochs_start, 50):
        ...
        for feats, iden in train_dataloader:
            ...
            if iterations%100 == 0:
                ...
                msg = "{}: Epoch: [{}/{}] ({}/{}) \t AvgLoss:{:.4f} \t C-Loss:{:.4f} \t LR : {:.8f} \t Margin : {:.4f} \t Accuracy : {:.4f}".format(time.ctime(), epochs, 50, iterations, len(train_dataloader), rmean_loss, loss.item(), get_lr(optimizer), resnet_model.module.get_m(), acc)
                print(msg)
