...
def process_file(segment: Pathlike, in_data: Pathlike):
    ...
    # print(f"*** {name} {duration} {output}")

    # return name, duration, str(output)


def prepare_asvspoof5(
    ...
    rliste = open(in_data / "ASVspoof5.train.metadata.txt")
    for line in rliste:
    ...
    with ProcessPoolExecutor(num_jobs) as ex:
        ...
        for segment in os.scandir(Path(in_data / "flac_T")):
