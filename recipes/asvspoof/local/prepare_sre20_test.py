...
def prepare_sre20_test(in_data: Pathlike, out_data: Pathlike, jobs: int):
    ...
    with ProcessPoolExecutor(jobs) as ex:
        ...
        for future in tqdm(futures, desc="Processing SRE20 Test"):
    ...
    with open(in_data / "docs" / "2020_cts_challenge_trials.tsv", "r") as f:
        for line in f:
            if counter > 0:
                line = line.rstrip()
                line = line.split("\t")
                enrollment = line[0]
                test = line[1]
                trials.write(f"{enrollment} {test} target\n")
            counter += 1
...
if __name__ == "__main__":
    ...
    prepare_sre20_test(args.in_data, args.out_data, args.thread)
