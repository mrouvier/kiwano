...
def process_file(segment: Pathlike, sampling_frequency: int, out_data: Pathlike):
    ...
    if sampling_frequency != 16000:
        out = Path( out_data / str(segment).split("/")[-4]  / str(segment).split("/")[-3] / str(segment).split("/")[-2]  )
        out.mkdir(parents=True, exist_ok=True)

        output = Path( out_data / str(segment).split("/")[-4]  / str(segment).split("/")[-3] / str(segment).split("/")[-2] /  str(segment).split("/")[-1]  )
...
def prepare_musan(in_data: Pathlike, out_data: Pathlike, sampling_frequency: int, num_jobs: int):
