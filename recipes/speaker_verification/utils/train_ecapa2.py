...
if __name__ == '__main__':
    ...
    if idr_torch.rank == 0:
        ...
        print(">>> Training on ", len(idr_torch.nodelist), " nodes and ", idr_torch.size, " processes, master node is ", MASTER_ADDR)
    ...
    train_dataloader = DataLoader(training_data, batch_size=16, drop_last=True, shuffle=False, num_workers=15, sampler=train_sampler, pin_memory=True)
    iterator = iter(train_dataloader)


    ecapa2_model = ECAPA2V3SubCenter(num_classes=18000)
    if args.checkpoint:
        ecapa2_model.load_state_dict(  checkpoint["model"]  )
    ecapa2_model.to(gpu)
    ecapa2_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(ecapa2_model)

    ecapa2_model = torch.nn.parallel.DistributedDataParallel(ecapa2_model, device_ids=[idr_torch.local_rank])

    optimizer = torch.optim.SGD([{'params':ecapa2_model.module.preecapa2.parameters(), 'weight_decay':0.0001, 'lr':0.00001},{'params':ecapa2_model.module.temporal_pooling.parameters(), 'weight_decay':0.0001, 'lr':0.00001},{'params':ecapa2_model.module.embedding.parameters(), 'weight_decay':0.0001, 'lr':0.00001},{'params':ecapa2_model.module.output.parameters(), 'lr':0.00001}], momentum=0.9)
    ...
    scheduler = WarmupPlateauScheduler(optimizer, max_epochs=150, initial_lr=0.1, warm_up_epoch=5, plateau_epoch=15, patience=10, factor = 5, margin_loss = 0.3)
    if args.checkpoint:
    ...
    for epochs in range(epochs_start, 150):
        ...
        for feats, iden in train_dataloader:
            ...
            if iterations%100 == 0:
                msg = "{}: Epoch: [{}/{}] ({}/{}) \t AvgLoss:{:.4f} \t C-Loss:{:.4f} \t LR : {:.8f} \t Margin : {:.4f}".format(time.ctime(), epochs, 150, iterations, len(train_dataloader), rmean_loss, loss.item(), get_lr(optimizer), ecapa2_model.module.get_m())
                print(msg)
        ...
        if dist.get_rank() == 0:
            checkpoint = {
                ...
                "config": ecapa2_model.extra_repr(),
            }
