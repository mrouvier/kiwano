...
class SNorm:
    ...
    def compute_v(self, xvector):
        ...
        return np.array(
            [
                self.computeStrategy.scoring_xvector(xvector, impostor)
    ...
    def compute_score(self, xvectorEnrollment, xvectorTest, enrollmentName, testName):
        ...
        return (
            (score - self.mean_std[enrollmentName][0])
            / (self.mean_std[enrollmentName][1])
            + (score - self.mean_std[testName][0]) / (self.mean_std[testName][1])
        ) * 0.5
    ...
    def compute_norm(self):
        ...
        for names in self.keys:
            ...
            print(
                ...
                + str(
                    self.compute_score(
                        self.xvectors_enrollment[enrollmentName],
                        self.xvectors_test[testName],
                        enrollmentName,
                        testName,
                    )
                )
