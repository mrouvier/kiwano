...
class AAMsoftmax(nn.Module):
    def __init__(self, n_class, loss_margin, loss_scale):
        ...
        self.weight = torch.nn.Parameter(
            torch.FloatTensor(n_class, 192), requires_grad=True
        )
    ...
    def _smooth_one_hot(targets: torch.Tensor, n_classes: int, coeff1=0.0, coeff2=0.0):
        ...
        with torch.no_grad():
            targets = (
                ...
                .scatter_(1, targets.data.unsqueeze(1), 1.0 - coeff1 - coeff2)
            )
        return targets
    ...
    def _jeffreys_one_cold(targets: torch.Tensor, n_classes: int):
        with torch.no_grad():
            targets = (
                torch.empty(size=(targets.size(0), n_classes), device=targets.device)
                .fill_(1)
                .scatter_(1, targets.data.unsqueeze(1), 0.0)
            )
        return targets
    ...
    def _jeffreys_one_hot(targets: torch.Tensor, n_classes: int):
        with torch.no_grad():
            targets = (
                torch.empty(size=(targets.size(0), n_classes), device=targets.device)
                .fill_(0)
                .scatter_(1, targets.data.unsqueeze(1), 1.0)
            )
        return targets

    def forward(self, inputs: torch.Tensor, targets: torch.Tensor):
        targets1 = JeffreysLoss._smooth_one_hot(
            targets, inputs.size(-1), self.coeff1, self.coeff2
        )
        ...
        # Jeffreys part 2
        lsmsm = lsm * sm
        targets21 = JeffreysLoss._jeffreys_one_cold(
            targets,
            inputs.size(-1),
        )
        loss1 = (targets21 * lsmsm).sum(-1)

        targets22 = JeffreysLoss._jeffreys_one_hot(
            targets,
            inputs.size(-1),
        )
